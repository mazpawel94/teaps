    ...
    <div class="summary">
      ...
      <div class="inputs">
        ...
        <input
          ...
          min="0"
          name="five-zlotys"
          v-model="coins[8].amount"
          @input="emitSum"
        />
        <input
          type="number"
          name="ten-zlotys"
          v-model="coins[9].amount"
          @input="emitSum"
        />
      </div>
    </div>
    <div class="pig">
      <div class="amount">{{ beautyAmount(sum()) }}</div>
      <img src="../img/piggy-bank-1001599_640.png" alt="" />
...
export default {
  ...
  async created() {
    try {
      ...
      const coinsFromBase = await DBService.getMoneys();
      this.coins = coinsFromBase.sort((a, b) =>
        correctOrder.indexOf(a.coin) > correctOrder.indexOf(b.coin) ? 1 : -1
      );
